nums = tuple(int(line)*811589153 for line in open('inp').read().split('\n'))
nums_len = len(nums)
...
sum_inds = nums[indexes.index((ind_0+1000)%nums_len)] + nums[indexes.index((ind_0+2000)%nums_len)] + nums[indexes.index((ind_0+3000)%nums_len)]
print(sum_inds)
